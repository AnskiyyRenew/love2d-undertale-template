...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    //bg
    vec3 acolor = bg(uv)*(2.-abs(uv.y*2.));
    ...
    for(float i=0.; i<total; i++){
	    ...
	    acolor += rectColor * rect * pos.z/maxSize;
    }
    
	fragColor = vec4(acolor, 1.0);
}
]]
local st = ImportFile("ShaderToy")
local shader = st.convert(shader_code)
bg:SetShaders({shader.shader})
